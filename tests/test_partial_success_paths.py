...
def test_llm_node_uses_unified_signal_integrity_for_confidence(monkeypatch) -> None:
    ...
    # Competitor intelligence removed — no external_fetch penalty.
    expected_base = 0.25
    assert diagnostics.get("signal_integrity_scores", {}).get("Unified_integrity_score") == pytest.approx(
        expected_base, abs=1e-6,
    )
    assert diagnostics.get("confidence_score") == pytest.approx(expected_base, abs=1e-6)
    assert payload["competitive_analysis"]["confidence"] == pytest.approx(expected_base, abs=1e-6)
...
def test_llm_node_degraded_modes_emit_schema_valid_withheld_recommendations(
    ...
    for item in items:
        ...
        assert not lower.startswith("conditional:"), f"Unexpected prefix in: {item}"
        assert any(term in lower for term in context_terms)


def test_llm_node_degraded_mode_uses_self_analysis_fallback(monkeypatch) -> None:
    """Competitor intelligence removed — degraded mode always uses self-analysis recommendations."""
    monkeypatch.setattr("agent.nodes.llm_node.load_env_files", lambda: None)
    monkeypatch.setattr("agent.nodes.llm_node._build_adapter", lambda **_kw: object())
    monkeypatch.setattr(
        "agent.nodes.llm_node.generate_with_retry",
    ...
    assert "revenue" in immediate or "retention" in immediate or "churn" in immediate
